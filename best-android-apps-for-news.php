...
//  Adding Action
add_action('plugins_loaded','news_display_widget');

//  Display Widget
function news_display_widget() {
    register_sidebar_widget(__('Best Android apps for News Widget'),'news_rawapps_widget');
    register_widget_control('Best Android apps for News Widget','news_widget_options');

}
//  Creating RawApps  widget
function news_rawapps_widget($args) {
    extract($args);
    $options=get_option("news_rawapps_widget");
    if (!is_array($options)) {
	    $options=array(
		    'title'=>'Top News Android Apps',
		    'show_description'=>'1',
		    'show_credit'=>'1',
		    'show_ratings'=>'1',
		    'show_noratings'=>'1',
   ...
   //   Widget template
    function news_rawapps_widget_template() {include_once(WP_PLUGIN_DIR."/best-android-apps-for-news/rawapps_widget_template.php");}

   //   Widget Control Panel
    function news_widget_options()
    {
	...
	if ($_POST['news-rawapps-widget-submit']) {
	    ...
	    update_option("news_rawapps_widget",$options);
	}
	//Form
	...
	echo "<input type='hidden' id='news-rawapps-widget-submit' name='news-rawapps-widget-submit' value='1' />";
